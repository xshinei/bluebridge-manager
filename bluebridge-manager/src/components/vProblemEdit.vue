...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    computed: {
      imageList() {
        if(!this.values.imageList) {
          return [];
        }

        return this.values.imageList.map(item => {
          return {
            name: item.url,
            url: item.url
          }
    ...
    created() {
      ...
      this.RULES = {
        ...
        sortId: [
          {required: true, pattern: /.+/, message: '排序Id不能为空！', trigger: 'blur' }
        ],
        mainDescription: [
    ...
    methods: {
      ...
      async handleSubmit() {
        ...
        try {
          ...
          if(validateResult) {
            ...
            const result = await axios.post('/problem/edit', params);
            const data = result.data;
      ...
      handleUploadPreview(file) {
        this.modalInfo.imgUrl = `/images/problem/${file.name}`;
